...
INS_insurgentBrain = addMissionEventHandler [ "EachFrame",
{
	...
	// process group strategy
	{
		...
		switch ( _tasking ) do {
			case "GARRISON": {
				private _units = _interestingUnits select { side _x != (_leader getVariable "ins_side") && count ([_x,units _group,250] call CBA_fnc_getNearest) > 0 && ( _group knowsAbout _x ) > 0.5 };
			...
			case "PATROL": {
				private _units = _interestingUnits select { side _x != (_leader getVariable "ins_side") && count ([_x,units _group,1000] call CBA_fnc_getNearest) > 0 && ( _group knowsAbout _x ) > 0.5 };
