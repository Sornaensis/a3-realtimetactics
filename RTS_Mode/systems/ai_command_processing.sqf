{ 
	...
	if ( _status == "WAITING" && (count _commands) > 0 ) then {
		...
		if ( (vehicle (leader _group)) != (leader _group) && ( (group (driver (vehicle (leader _group)))) == _group ) ) then {
			[(driver (vehicle (leader _group))), _pos, _type, _speed, _behaviour] spawn {
				params ["_unit", "_pos", "_type", "_speed", "_behaviour"];
				_unit enableAi "MOVE";
				...
				while { alive _unit && ((count _commands) > 0) && !_complete} do {
					...
					_future = time + 2;
					waitUntil { speed (vehicle _unit) != 0 || (time > _future && !RTS_paused) };
					sleep 3;
					waitUntil { (speed (vehicle _unit) == 0 && !RTS_paused) || ([_unit, _pos] call CBA_fnc_getDistance) < 2 || !((count ((group _unit) getVariable ["commands", []])) > 0) || !(alive _unit) };
					_complete = true;
			...
			if ( _type == "GETIN" || _type == "MOUNT" ) then {
				...
				[_group, _pos, _behaviour, _combat, _speed, _form] spawn {
					params ["_group", "_pos", "_behaviour", "_combat", "_speed", "_form"];
					...
					while { ( ( count ((units _group) select { alive _x }) ) > 0 ) && ((count _commands) > 0) && !_complete} do {
						...
						waitUntil { ([getPosAtl (leader _group), _pos] call CBA_fnc_getDistance) < 10 || !((count (_group getVariable ["commands", []])) > 0) || !(alive (leader _group) ) };
